...
ccl_device uint sobol_dimension(__thread_space KernelGlobals *kg, int index, int dimension)
{
  uint result = 0;
...
ccl_device_forceinline float path_rng_1D(
    __thread_space KernelGlobals *kg, uint rng_hash, int sample, int num_samples, int dimension)
{
...
ccl_device_forceinline void path_rng_2D(__thread_space KernelGlobals *kg,
                                        uint rng_hash,
                                        int sample,
                                        int num_samples,
                                        int dimension,
                                        __thread_space float *fx,
                                        __thread_space float *fy)
{
#ifdef __DEBUG_CORRELATION__
  *fx = (float)drand48();
  *fy = (float)drand48();
...
ccl_device_inline void path_rng_init(__thread_space KernelGlobals *kg,
                                     int sample,
                                     int num_samples,
                                     __thread_space uint *rng_hash,
                                     int x,
                                     int y,
                                     __thread_space float *fx,
                                     __thread_space float *fy)
...
ccl_device uint lcg_step_uint(__thread_space uint *rng)
{
  /* implicit mod 2^32 */
...
ccl_device float lcg_step_float(__thread_space uint *rng)
{
  /* implicit mod 2^32 */
  *rng = (1103515245 * (*rng) + 12345);
...
ccl_device_inline float path_state_rng_1D(__thread_space KernelGlobals *kg,
                                          __device_space const ccl_addr_space PathState *state,
                                          int dimension)
{
  return path_rng_1D(
...
ccl_device_inline void path_state_rng_2D(
    __thread_space KernelGlobals *kg, __device_space const ccl_addr_space PathState *state, int dimension, __thread_space float *fx, __thread_space float *fy)
{
  path_rng_2D(kg,
              state->rng_hash,
              state->sample,
...
ccl_device_inline float path_state_rng_1D_hash(__thread_space KernelGlobals *kg,
                                               __device_space const ccl_addr_space PathState *state,
                                               uint hash)
{
...
ccl_device_inline float path_branched_rng_1D(__thread_space KernelGlobals *kg,
                                             uint rng_hash,
                                             __device_space const ccl_addr_space PathState *state,
                                             int branch,
                                             int num_branches,
...
ccl_device_inline void path_branched_rng_2D(__thread_space KernelGlobals *kg,
                                            uint rng_hash,
                                            __device_space const ccl_addr_space PathState *state,
                                            int branch,
                                            int num_branches,
 ...
 */
ccl_device_inline float path_state_rng_light_termination(__thread_space KernelGlobals *kg,
                                                         __device_space const ccl_addr_space PathState *state)
{
...
ccl_device_inline float path_branched_rng_light_termination(__thread_space KernelGlobals *kg,
                                                            uint rng_hash,
                                                            __device_space const ccl_addr_space PathState *state,
                                                            int branch,
                                                            int num_branches)
...
ccl_device_inline uint lcg_state_init(__device_space PathState *state, uint scramble)
{
  return lcg_init(state->rng_hash + state->rng_offset + state->sample * scramble);
}

ccl_device_inline uint lcg_state_init_addrspace(__device_space ccl_addr_space PathState *state, uint scramble)
{
  return lcg_init(state->rng_hash + state->rng_offset + state->sample * scramble);
}

ccl_device float lcg_step_float_addrspace(__thread_space ccl_addr_space uint *rng)
{
  /* Implicit mod 2^32 */
  *rng = (1103515245 * (*rng) + 12345);
  return (float)*rng * (1.0f / (float)0xFFFFFFFF);
